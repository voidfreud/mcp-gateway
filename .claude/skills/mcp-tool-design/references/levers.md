...
Code mode (#13): `[meta] enabled = true` in config.toml mounts `/meta/mcp` with three meta-tools (`search`/`get_schema`/`execute`) over the whole catalog. Their text is CODE-authored (in `meta.py`), not config-authored — no override lever applies to them; retuning them is a code change. But they change the field: `search` matches on the EXPOSED name/title/description/param docs, and `get_schema`/`execute` take exposed names — so a rename ripples into code-mode discoverability too. Toggle takes a restart; off = endpoint absent.
