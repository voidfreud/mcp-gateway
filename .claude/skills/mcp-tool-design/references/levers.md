...
- Name uniqueness spans EVERY stored override entry, not just broadcast tools: a DISABLED tool's name and a *dangling* override (its `original` no longer in the captured baseline — the backend renamed the tool upstream) still occupy transform target names, and a save that would duplicate one is a 400 telling you to reset/rename the stale entry first. When a baseline refresh (#43) logs `override_no_match`, migrate that override's text to the tool's NEW original and reset the old entry — the old text silently stops applying otherwise.
