...
| # | Action | Targets (audit refs) | Effort | Acceptance criteria |
|---|--------|----------------------|--------|---------------------|
| A1 | **Fix the gateway-authored compliance violations** (config/user-reachable, smallest): prompt rename vs live-backend collision check (transform build + admin collision path); align virtual inputSchema null-`anyOf` with runtime acceptance; reject `NaN` defaults in `VirtualInput`/`ParamOverride.default`/`static_args` | #325, #326, #327 | S | One regression test per bug; all three red→green |
| A2 | **JSON Schema 2020-12 meta-validation of gateway-authored schemas** — validate every virtual tool's `inputSchema` against the official meta-schema in the test suite (catches the #326/#327 classes forever; the conformance suite can never see config-authored schemas) | #326, #327 | S | pytest green; `default = nan` / null-mismatch configs rejected |
| A3 | **Full applicable official conformance suite as the CI gate** — extend `tests/conformance/run_official.py` `SCENARIOS` to every advertised-capability scenario (tools-call-* variants, resources-*, prompts-*, logging-set-level, json-schema-2020-12, transport); extend the fixture with image/audio/mixed/embedded-resource tools; explicit skip-with-reason matrix for non-advertised capabilities (sampling, elicitation, subscriptions, completion); fix what goes red (#322–#324 code-0 errors, #341 unknown-tool semantics, #342 progress) | #322–#324, #341, #342; CI conformance coverage table | M–L | mcp-contract CI job green on the full applicable matrix; skip list documented in `run_official.py` |
| A4 | **Initialize-response + capability snapshot tests per endpoint** — assert `protocolVersion`, `serverInfo.name`/`version` (report the gateway's own version, not fastmcp's), capability truthfulness (virtual endpoint must not advertise prompts/resources it cannot serve; logging/ui-extension advertisement decisions) | #335, #351, #352, #353 | S–M | Snapshots green; serverInfo identity fixed; advertisement matches implementation |
| A5 | **Error-code + session/timeout contract tests** — unknown prompt → `-32602`; unhandled handler exception → `-32603`; MCP-Session-Id missing/invalid → 400/404; DELETE termination; initialize-handshake deadline; broadcast tool-call timeout | #322, #323, #324, #328, #330, #333 | M | Contract tests red→green; the two missing timeouts wired |
| A6 | **Fix the fastmcp-compat canary and keep SDK tripwires honest** — repair `fastmcp-compat.yml` (never syncs the workspace / PATH, so it can never pass — CI #168); assert wire effects (e.g. initialize shows `listChanged: false`) rather than private attributes only | CI #168; #90; tripwire tests | S | fastmcp-compat lane green on nightly; private-API usage covered by wire-effect assertions |
| A7 | **First-run self-install of the daemon — single-command setup.** Make the app install the macOS LaunchAgent itself so `uv tool install mcp-gateway && mcp-gateway` is the whole setup (uv has no post-install hooks by design, so the *program* owns the service install). Reuse install.sh's plist-rendering logic inside the app; plist targets the stable uv shim (`~/.local/bin/mcp-gateway`) so `uv tool upgrade` survives. Locked design decisions: (1) **interactive prompt on first run — never silently daemonize** — plus `--install-service` / `--foreground` flags for scripts and CI; (2) **uninstall symmetry**: `mcp-gateway` must remove the service before `uv tool uninstall`, otherwise a dead LaunchAgent crash-loops at every login — README must state the order; (3) **platform scope**: LaunchAgent is macOS-only — polite error elsewhere, systemd-user units deferred. Ships with tests: service install/remove round-trip, plist rendering, state-dir creation (absorbs A0/#165 at the source), upgrade stability (shim target), prompt/flag behavior. `install.sh` / `just install` become thin legacy wrappers for the checkout workflow. | #165, A0; docs/installation.md; justfile | M | Fresh machine: one command + one prompt → daemon running, /health green; service removal + `uv tool uninstall` leaves zero launchd residue; tests green |
...
**A7 — MANDATORY availability-model decision (must be chosen and recorded before A7 implementation starts; A7 is blocked until then):**

| Option | What it is | Costs / tradeoffs | Fits if… |
|--------|------------|-------------------|----------|
| **A. Resident daemon (status quo)** | launchd `KeepAlive: true`, always-on process, warm backend sessions | All the complexity this audit catalogued: lifecycle races (#150/#151), upgrade-while-running dance, crash-loop guard (req 7), uninstall residue; holds stdio backend child processes open indefinitely | Always-available service is the product; zero client-visible latency is required |
...
**Decision mandate:** A7 is **not implemented until one option is explicitly chosen and recorded as a decision (per the repo's ADR process, docs/decisions/README.md), with rationale — never left to implementation accident.** The chosen option determines which A7 requirements apply (e.g., B makes req 7 largely moot and softens req 2; C deletes the whole requirement list). **Default if no strong opinion is expressed at decision time: Option A** (status quo, the documented current design), with B documented as the zero-residency upgrade path — but the choice itself must still be made explicitly and recorded.
...
| # | File:Line | Issue |
|---|-----------|-------|
| 149 | `server.py:425,443`, `config_loader.py:64,924` | **Auth bypass via prefix exemption.** BearerAuth exempts by `path.startswith(("/health", "/ready"))` but backend names are reserved only by exact match — a backend named `health-check`/`ready-api` mounts at `/health-check/mcp` and is served with NO bearer challenge. (SrvA.1, Security.1) |
...
| 164 | `virtual_tools.py:603–604 vs 274–299` | **Advertised input schema rejects its own `null`.** Optional inputs without defaults advertise `anyOf: [type, null]`, but `_validate_arguments` rejects explicit null → a conforming MCP client gets a tool error. (VTools.1, Tvirt.6) |
...
| 325 | Prompts (C3.15) | **Prompt rename shadows a live backend prompt → duplicate broadcast names** violate the spec's unique-identifier contract; the live prompt becomes unreachable via prompts/get. Both the transform duplicate check (config_loader.py:1280–1290) and the admin collision check (admin.py:1240–1257, stale against live backends) miss override-vs-live collisions. (= #200, now classified as a spec violation.) |
| 326 | Tools (C2.11) | **Advertised inputSchema accepts `null` the runtime rejects.** Optional no-default virtual inputs advertise `anyOf: [type, null]` (virtual_tools.py:603–604) but `_validate_arguments` refuses explicit null (274–299) — a conforming client following the advertised schema gets a tool error. (= #164, confirmed High.) |
| 327 | Tools (C2.5) | **TOML `default = nan` corrupts the tool contract.** Config-reachable NaN passes validation, pydantic serializes the schema default as `null` for `type: number` (empirically verified), and calls inject null into backends. Not invalid JSON on the wire (correcting #206's mechanism), but the advertised schema contradicts its declared type. |
