# ADR-0002: One gateway MCP endpoint per backend

**Status:** Accepted

**Decision date:** 2026-07-01

**Deciding issue / PR:** [#29](https://github.com/voidfreud/mcp-gateway/issues/29),
[#9](https://github.com/voidfreud/mcp-gateway/issues/9), and
[#34](https://github.com/voidfreud/mcp-gateway/pull/34)

## Context

The gateway originally aggregated all backends behind ONE `create_proxy` served
at a single `/mcp` endpoint. Claude Code was one observed client: it truncates
each MCP server's `instructions` at ~2KB. With one server, every backend's
server-level instructions were composed into that single budget — measured at
~2306–2332 B for the configured backend set, over the cap, so the tail was
silently truncated, and each added backend made it worse
...
session-manager lifespan via an `AsyncExitStack`. Each proxy carries only its
own backend's `instructions` and its own session strategy. A client registers
the endpoints it needs; Claude Code's `gateway-<backend>` registrations are an
example integration, not the product boundary.

## Consequences

- Each backend gets a separate server-instructions budget; Claude Code receives
  its full ~2KB budget without truncation.
...
- **User-visible:** clients discover one endpoint per backend instead of one
  aggregate endpoint. Client-specific registrations may need a one-time update.
